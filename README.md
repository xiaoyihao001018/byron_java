...
1. **秒杀服务**
```java:src/main/java/org/example/service/SeckillService.java
startLine: 19
endLine: 35
```
...
2. **消息生产者**
```java:src/main/java/org/example/mq/producer/SeckillProducer.java
startLine: 25
endLine: 41
```

3. **高并发测试**
```java:src/test/java/org/example/service/SeckillServiceTest.java
startLine: 46
endLine: 74
```
...
2. **防止重复下单**
```java
String orderKey = String.format("order:user:%d:product:%d", userId, productId);
Boolean success = redisTemplate.opsForValue().setIfAbsent(orderKey, "1", 24, TimeUnit.HOURS);
if (!success) {
